...
public class RedisCli {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(RedisCli.class);
    ...
    private synchronized void initPool() {
        if (pool == null) {
            pool = new JedisPool(poolConfig, host, port, 20000);
        }
    }
    ...
    public boolean exists(final Jedis jedis, final String key) {
        boolean flag = false;
        ReadWriteLock lock = new ReentrantReadWriteLock();
        Lock readLock = lock.readLock();
        readLock.lock();
